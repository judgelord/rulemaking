...
comments <- tibble( path = list.files( here::here("comment_text", agency, docket), full.names = T) ) 

# apply comment_tengrams function
d <- comment_tengrams(nprm, 
                      comments %>% top_n(50) # just a few comments for now 
                      )
```
...
2. `reuse` contains a data frame for each other comment:
    + `document_id2`  
...
nprm_percent_match <- d %>% 
  select(-reuse) %>% # select just the doc name and text table (otherwise unnest duplicates the reuse table for every observation)
  unnest(text) %>%
  group_by(document_id) %>% 
  summarise(percent_match = sum(nprm_phrase)/n() ) 
```
...
nprm_percent_match %>% 
  ...
  ggplot() + 
  aes(x = document_id, y = percent_match) + 
  geom_col() + 
...
d %>% 
  ...
  select(text) %>% 
  unnest(text) %>% 
  rename(match = nprm_phrase) %>% 
  tengram_match_to_text() %>% kable()
```
...
Just as `nprm_phrase` indicates whether each word in a comment was part of a tengram that appeared in the NPRM, there is a column `reuse` indicating whether each word was part of a tengram that also appeared in each other comment in the data.
...
comment_percent_match <- d %>% 
  ...
  select(-text) %>% # select just the doc name and reuse table (otherwise unnest duplicates the text table for every observation)
...
```{r comment_percent_match_plot, fig.width=9.5, fig.height=7.5}
...
Partial matches are often but not always exactly symmetrical; the percent of comment x in comment y may not be the same as the percent of y in x. For example, 100% of A  is in B, but only 50% of B is in A.
