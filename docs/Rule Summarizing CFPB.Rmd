...
clean <- . %>%  
  ...
  filter(text != "") %>% #filter blank strings
    ...
    mutate(first_char = str_sub(text, 1, 1) %>% as.numeric() ) %>%  
  # identify headings as lines with a period in the beginning
...
text_sentences %<>% select(textrank_id, sentence)
...
summary <- summarizeText(ruleText)
