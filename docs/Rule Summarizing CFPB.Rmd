...
# split out sections
section <- . %>% 
  mutate(section = text %>% str_extract("^SUMMARY:|^([A-Z]*\\.|[1-9]\\.).*") %>% 
           str_sub(1,80) ) %>% 
...
# consider using this clean function instead of the last three lines. https://github.com/judgelord/rulemaking/blob/master/functions/clean_string.R
...
summarize_sections <- function(text, n_sentences, max_sentences) {
  ...
  section_summary <- tibble(
    ...
    summary = text_summary %>% map_chr(possibly(pull_sentences, otherwise = " ") ) ) 
