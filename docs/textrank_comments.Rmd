...
summarizeText <- function(document_ids, text, max_sentences) {
...
summarize_comments <- function(data, n_sentences = 2, max_sentences = 100) {
   ...
   # data %<>%
   #   mutate(text = str_remove_all(text, "Docket no\\. Cfp consumer financial protection bureau g street, nw\\."))
...
# comment_summary %>%
#   filter(comment == "CFPB-2016-0025-3913-1") %>%
#   select(summary)
