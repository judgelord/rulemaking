...
custom_stop_words <- tibble( 
  words = c("cfpb", "CFPB", "Payday, Vehicle Title, and Certain High-Cost Installment Loans")) %>%
  # words = c("cfpb", "CFPB", "Payday, Vehicle Title, and Certain High-Cost Installment Loans", "Consumer Financial Protection Bureau")) %>%
  mutate(words = words)%>%
...
summarizeText <- function(text, max_sentences, custom_stop_words) {
  ...
  # sentences %<>%
  #   mutate(sentences = str_remove_all(sentences, "Consumer Financial Protection Bureau"))
...
summarize_comments <- function(text, n_sentences = 2, max_sentences = 100, custom_stop_words) {
