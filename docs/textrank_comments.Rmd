...
custom_stop_words <- tibble( words = c("cfpb", "CFPB", "Payday, Vehicle Title, and Certain High-Cost Installment Loans")) %>%
  mutate(words = words)%>%
  unnest_tokens(output = sentences, input = words) %>%
  distinct()


#agency_text <- c("cfpb", "CFPB", "Payday, Vehicle Title, and Certain High-Cost Installment Loans")

#data <- d

summarizeText <- function(text, max_sentences, custom_stop_words) {
  ...
  words <- unnest_tokens(sentences, output = word, input = 'sentences') %>% 
    ...
    anti_join(custom_stop_words, by = character())
    # anti_join(agency_text, copy = TRUE, by = character()) #Creates bad summaries
...
summarize_comments <- function(data, n_sentences = 2, max_sentences = 100, custom_stop_words) {
...
comment_summary <- summarize_comments(d, n_sentences = 3, max_sentences = 50, custom_stop_words )
```
