...
prr10 %>% 
  ...
  rename(`Words in Final Rule` = `FALSE`, 
         `Words not in Final Rule` = `TRUE`)  %>% 
  kablebox()
...
frr10 %>% 
  ...
  select(-word_count) %>% 
  spread(key = new, value = text) %>% 
  rename(`Words from Proposed Rule` = `FALSE`, 
         `Words not from Proposed Rule` = `TRUE`)  %>%
  kablebox()
```
