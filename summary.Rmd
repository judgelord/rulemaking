...
d %<>% arrange(desc(numberOfCommentsReceived))  %>% 
  ...
  mutate(mass = ifelse(numberOfCommentsReceived > 1, "Mass Comments", "Unique Comments"))
...
d %<>%  
  mutate(position = ifelse(grepl(" support ", commentText), "support", NA )) %>% 
  mutate(position = ifelse(grepl(" oppose ", commentText), "oppose", position )) %>% 
    mutate(position = ifelse(grepl(" support ", commentText) & grepl(" oppose ", commentText), "support and oppose", NA )) 
```

```{r comments-mass-vs-unique, fig.height=3, fig.width=6}
d %>% 
  filter(year > 2005) %>% 
  group_by(year, mass) %>% 
  ...
  labs(x = "", 
       y = paste("Public Comments, N =", round(sum(d$numberOfCommentsReceived)/1000000,1), "million"),
       fill = "") + 
  theme_minimal() + 
...
```{r comments-support-vs-oppose, fig.height=3, fig.width=6, eval= FALSE}
d %>% 
  filter(year > 2005, !is.na(position)) %>% 
...
```{r comments-mass-support-vs-oppose, fig.height=3, fig.width=6}
d %>% 
  filter(year > 2005, !is.na(position)) %>% 
...
`r d$organization[which(d$numberOfCommentsReceied > 99 & !is.na(d$organization))]`

### American Petroleum Institute, breaking a perceived consensus
`r head(d[which(d$organization=="American Petroleum Institute")])`
