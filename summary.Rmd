...
```{r summary-table}
count <- d %>% 
  filter(year > 1993,!is.na(commentStartDate)) %>%
...
d %<>% arrange(desc(numberOfCommentsReceived))  %>% 
  ...
  mutate(mass = ifelse(numberOfCommentsReceived > 1, "Mass Comment Campaign", "Unique Comments"))
...
d %>% 
  ...
  labs(x = "", 
       y = paste("Total Public Comments, N =", round(sum(d$numberOfCommentsReceived)/100000, 1), "million"),
       fill = "") + 
  theme_minimal() + 
