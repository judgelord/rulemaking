...
```
```{r rules-ranked-comments-per-year, fig.height = 2}
...
```{r comments-per-year, fig.height=3, fig.width=6}
d %>% 
  filter(year > 2005) %>% 
  group_by(year) %>% 
...
```{r comments-mass-vs-unique, fig.height=3, fig.width=3}
d %>% 
  filter(year > 2005) %>% 
  group_by(year, mass) %>% 
  summarise(yearTotal = sum(numberOfCommentsReceived)) %>%
...
```{r comments-support-vs-oppose, fig.height=3, fig.width=4}
d %>% 
...
```{r comments-mass-support-vs-oppose, fig.height=3, fig.width=4}
d %>% 
